...
function onLoad() {
    ...
    chessCanvas2 = document.getElementById("chessCanvas2");
    chessCtx2 = chessCanvas2.getContext("2d");
    chessCanvas2.addEventListener("click", onClick);
    ...
    if (checkValidMovement(x, y) === true) {
        if (checkValidCapture(x, y) === true) {
            ...
            if (currentTeam === WHITE) {
                ...
                updateBlackCasualities();
                console.log(blackCasualities)
            } else {
                whiteCasualities[board.tiles[y][x].pieceType]++;
                updateWhiteCasualities();
                console.log(whiteCasualities)
            }
        }
...
function drawBoard2() {
    chessCtx2.fillStyle = "gray";
   // console.log("red")
    chessCtx2.fillRect(0, 0, BOARD_WIDTH*TILE_SIZE, BOARD_HEIGHT*TILE_SIZE);
...
function drawTile2(x, y, fillStyle) {
   // console.log(x)
    chessCtx2.fillStyle = fillStyle
    chessCtx2.fillRect(TILE_SIZE*x, TILE_SIZE*y, TILE_SIZE, TILE_SIZE);
}
    ...
    for (let i = 0; i < BOARD_HEIGHT; i++) {
        for (let j = 0; j < BOARD_WIDTH; j++) {
            if (board.tiles[i][j].team === EMPTY) continue;
                        ...
                        if (board.tiles[i][j].team === WHITE) {
                chessCtx2.fillStyle = "#FF0000";
            } else {
                chessCtx2.fillStyle = "#0000FF";
            }         
        
            chessCtx2.font = "38px Arial";

            let pieceType = board.tiles[i][j].pieceType;

            for (n = 0; n<6 ; n++){
                            
                if(whiteCasualities[n]>0){
                    

                    chessCtx2.fillText(piecesCharacters[n], TILE_SIZE*(j+1/8), TILE_SIZE*(i+4/5));

                    whiteCasualities[n]=0
            }
        }
        for (w = 0; w<6 ; w++){
                            
            if(blackCasualities[w]>0){
                

                chessCtx2.fillText(piecesCharacters[w], TILE_SIZE*(j+1/8), TILE_SIZE*(i+9/5));

                blackCasualities[w]=0
        }
    }

           // chessCtx2.fillText(piecesCharacters[pieceType], TILE_SIZE*(j+1/8), TILE_SIZE*(i+4/5));*/
        }
    }
}
...
//console.log(casualities)
    for (let i = QUEEN; i >= PAWN; i--) {
        ...
        if (none) {
            ...
            none = false;
            //console.log(piecesCharacters[i])
        } else {
            text.textContent += " - " + casualities[i] + " " + piecesCharacters[i];
        }
    }
